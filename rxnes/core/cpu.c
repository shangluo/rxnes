...
void cpu_mm_write( u16 addr, u8 *buf, u16 len )
{
    ...
    if ( ((addr >= 0x5000 && addr < 0x6000) || (addr >= 0x8000)) &&  c_rom->mapper != 0x0 )
    {
...
void cpu_handle_io( u16 addr )
{
	...
	else if (addr >= PPU_CTRL_REG1 && addr <= PPU_DATA)
	{
...
void cpu_mm_read( u16 addr, u8 *buf, u16 len )
{
	...
	if ((addr >= 0x5000 && addr < 0x6000) || (addr >= 0x8000))
	{
		// mapper read
		mapper_read(addr, buf, len);
		return;
	}
	
	if (addr == PPU_DATA || addr == PPU_SPR_DATA || addr == PPU_STATUS)
	{
...
void cpu_set_irq_pending()
{
	cpu_irq_pending = 1;
}

u8 cpu_is_irq_pending()
{
	return cpu_irq_pending;
}

void cpu_handle_irq(void)
{
	mapper_check_pending_irq();
	papu_check_pending_irq();

	// if irq pending
	if (!regs.SR.I && cpu_irq_pending)
	{
...
static u8 cpu_disassemble_intruction_internal(u16 addr, char *instruction, CPU_ADDRESSING_MODE mode, char *buf, int len)
{
	...
	case Absolute_X:
	case Absolute_Y:
		cpu_mm_read(addr + 1, (u8 *)&operand, 2);
		bytes = 3;
		break;
	default:
		break;
